# 词向量获取
- 在原作者开源的基础上，参考[https://github.com/smilealvin92/bert-utils](https://github.com/smilealvin92/bert-utils)， 先将原始的checkpoint模型转为pb的graph，然后进行词向量的计算。
- 增加extract_feature.py和graph.py文件，从倒数第二层提取每一个token的向量，然后对每个词的所有的token的向量取平均，得到该词的词向量，但是结果并不太如人意。
# albert_zh
